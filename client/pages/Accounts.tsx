...
export default function Accounts() {
  const roles = ROLE_ORDER.map((id) => allRoles.find((r) => r.id === id)!).filter(Boolean);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">Admin / Staff Accounts</h1>
        <p className="text-sm text-muted-foreground">Defined roles with clear permissions. Manage assignments and fine-tune in User Roles.</p>
      </div>
          ...
          <Card key={r.id}>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                ...
                <Link to="/roles"><Button size="sm" variant="outline">Manage in User Roles</Button></Link>
              </div>
            </CardContent>
          </Card>
        ))}
