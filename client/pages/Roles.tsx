...
import { roles as seedRoles, users as seedUsers, logs as seedLogs } from "./roles/data";
...
export default function Roles() {
  ...
  const [workingPerms, setWorkingPerms] = useState<RolePermissions>(() => clonePerms(selectedRole.permissions));
  ...
  const savePerms = () => {
    setRoles((prev) => prev.map((r) => (r.id === selectedRoleId ? { ...r, permissions: workingPerms } : r)));
    setLogs((l) => [{ id: `l-${Date.now()}`, timestamp: new Date().toISOString(), user: "Admin User", action: "Updated permissions", details: `${selectedRole.name}` }, ...l]);
    toast({ title: "Permissions updated" });
  };

  const assignRole = (userId: string, roleId: string) => {
    setUsers((prev) => prev.map((u) => (u.id === userId ? { ...u, roleId } : u)));
    setLogs((l) => [{ id: `l-${Date.now()}`, timestamp: new Date().toISOString(), user: "Admin User", action: "Assigned role", details: `${userId} → ${roleId}` }, ...l]);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Define permissions, assign roles, and audit changes.</p>
      </div>
      <Tabs defaultValue="permissions">
        ...
        <TabsContent value="permissions" className="space-y-3">
          ...
          <PermissionsEditor role={selectedRole} perms={workingPerms} onChange={setWorkingPerms} />
        </TabsContent>
