...
const statuses: StudentStatus[] = ["Current", "Freeze", "Concluded", "Not Completed", "Suspended", "Alumni"];
...
export function Directory({
  ...
  const courses = Array.from(new Set(data.map((d) => d.admission.course))).sort();
  const batches = Array.from(new Set(data.map((d) => d.admission.batch))).sort();
  const campuses = Array.from(new Set(data.map((d) => d.admission.campus))).sort();

  const filtered = useMemo(() => {
    const s = q.toLowerCase();
    return data.filter((d) =>
      (!s || d.name.toLowerCase().includes(s) || d.id.toLowerCase().includes(s) || d.admission.course.toLowerCase().includes(s)) &&
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-6">
        <Input placeholder="Search by name, ID, course…" value={q} onChange={(e) => setQ(e.target.value)} />
        ...
        <Select value={course} onValueChange={(v) => setCourse(v === "__all" ? "" : v)}>
        ...
        <Select value={batch} onValueChange={(v) => setBatch(v === "__all" ? "" : v)}>
        ...
        <Select value={campus} onValueChange={(v) => setCampus(v === "__all" ? "" : v)}>
        ...
        <Button variant="outline" onClick={() => { setQ(""); setStatus(""); setCourse(""); setBatch(""); setCampus(""); }}>Reset</Button>
      </div>
        ...
        <TableBody>
          {filtered.map((s) => (
            <TableRow key={s.id}>
              ...
              <TableCell>
                <Badge variant={paymentStatus(s) === "Overdue" ? "destructive" : paymentStatus(s) === "Paid" ? "default" : "secondary"}>{paymentStatus(s)}</Badge>
              </TableCell>
