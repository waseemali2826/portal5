...
import { ensureAttendance, markInstallmentPaid, nextUnpaidInstallment, paymentStatus } from "./types";

export function Profile({ student, onChange }: { student: StudentRecord; onChange: (next: StudentRecord) => void }) {
  ...
  const enrollCourse = () => {
    ...
    const courses = Array.from(new Set([...(student.enrolledCourses || []), newCourse.trim()]));
  ...
  const notify = (chan: "SMS" | "Email" | "WhatsApp") => {
    onChange({
      ...student,
      communications: [
        { id: `m-${Date.now()}`, channel: chan, message: `Admin message via ${chan}`, at: new Date().toISOString() },
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className="text-xs text-muted-foreground">{student.email} • {student.phone}</div>
        <div className="pt-2 flex items-center gap-2">
          <Badge>{student.status}</Badge>
          <Badge variant={paymentStatus(student) === "Overdue" ? "destructive" : paymentStatus(student) === "Paid" ? "default" : "secondary"}>{paymentStatus(student)}</Badge>
        </div>
      </div>
      <Separator />
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <div className="text-xs text-muted-foreground">Batch: {student.admission.batch} • Campus: {student.admission.campus}</div>
          <div className="text-xs">Date: {new Date(student.admission.date).toLocaleDateString()}</div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">Fee Structure</div>
          <div className="text-xs">Total: ₹{student.fee.total.toLocaleString()}</div>
          ...
          <div className="pt-2"><Button size="sm" onClick={collectFee}>Collect Fee Installment</Button></div>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Button variant="outline" onClick={transferBatch}>Transfer Batch</Button>
          <Label>Campus</Label>
          <Input value={campus} onChange={(e) => setCampus(e.target.value)} />
          <Button variant="outline" onClick={transferCampus}>Transfer Campus</Button>
        </div>
        <div className="space-y-2">
          <div className="text-xs text-muted-foreground">Cross-enrollment</div>
          <Label>New Course</Label>
          <Input placeholder="Course" value={newCourse} onChange={(e) => setNewCourse(e.target.value)} />
          <Button variant="outline" onClick={enrollCourse}>Enroll</Button>
          <div className="text-xs text-muted-foreground">Enrolled: {(student.enrolledCourses || []).join(", ") || "-"}</div>
        </div>
      </div>
          ...
          {student.documents.map((d, idx) => (
            <div key={idx} className="flex items-center justify-between">
              ...
              <div className="space-x-2">
                ...
                <Button size="sm" variant="outline" onClick={() => onChange({ ...student, documents: student.documents.map((x, i) => (i === idx ? { ...x, verified: !x.verified } : x)) })}>Toggle Verify</Button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <div className="space-x-2">
            <Button size="sm" onClick={() => markAttendance(true)}>Present</Button>
            <Button size="sm" variant="outline" onClick={() => markAttendance(false)}>Absent</Button>
          </div>
        </div>
        <div className="space-y-2">
          <div className="text-xs text-muted-foreground">Communications</div>
          <div className="space-x-2">
            <Button size="sm" variant="outline" onClick={() => notify("SMS")}>Send SMS</Button>
            <Button size="sm" variant="outline" onClick={() => notify("Email")}>Send Email</Button>
            <Button size="sm" variant="outline" onClick={() => notify("WhatsApp")}>Send WhatsApp</Button>
          </div>
          <div className="rounded border p-2 text-xs space-y-1 max-h-40 overflow-auto">
            {student.communications.map((c) => (
              <div key={c.id} className="flex justify-between">
                <span>{c.channel} • {new Date(c.at).toLocaleString()}</span>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="outline" onClick={suspend}>Suspend</Button>
        <Button variant="outline" onClick={conclude}>Conclude (Alumni)</Button>
        <Button variant="destructive" onClick={dropout}>Not Completed</Button>
        <Button variant="outline" onClick={requestCertificate}>Request Certificate Approval</Button>
      </div>
    </div>
  );
}
